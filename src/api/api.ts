...
export const api = {
  auth: {
    authenticate: async (username: string, password: string): Promise<AxiosResponse<AuthResponse, any>> => {
        return await axios.post(AUTH_URL + "/authenticate", { username, password });
    },
  },
  ...
  patient: {
    ...
    getPagedFiltered: async (parameters: FilterParameters, pageNumber: number = 0, pageSize: number = 6): Promise<AxiosResponse<PagedResponse<Patient>, any>> => {
    ...
    add: async (payload: Patient): Promise<AxiosResponse<Patient, any>> => {
      ...
      patient.lastEditingUser = `${user?.secondname} ${user?.firstname} ${user?.patronymic || ''}`
  ...
  statistics: {
    getPatientCount: async (): Promise<AxiosResponse<PatientCountResponse, any>> => {
      return await axiosInstance.get(`${STATSTICS_URL}/count`);
    },

    getAgeCategoryCounts: async (): Promise<AxiosResponse<AgeCategoryResponse[], any>> => {
      return await axiosInstance.get(`${STATSTICS_URL}/age`);
    },

    getDominantHandCounts: async (): Promise<AxiosResponse<DominantHandCountsResponse, any>> => {
      return await axiosInstance.get(`${STATSTICS_URL}/dominant-hand`);
    },

    getGenderCounts: async (): Promise<AxiosResponse<GenderCountsResponse, any>> => {
      return await axiosInstance.get(`${STATSTICS_URL}/gender`);
    },

    getIcdCounts: async (): Promise<AxiosResponse<IcdCountsResponse[], any>> => {
      return await axiosInstance.get(`${STATSTICS_URL}/icd`);
    },

    getNationalityCounts: async (): Promise<AxiosResponse<NationalityCountsResponse[], any>> => {
      return await axiosInstance.get(`${STATSTICS_URL}/nationality`);
    },

    getRegionCounts: async (): Promise<AxiosResponse<RegionCountsResponse[], any>> => {
      return await axiosInstance.get(`${STATSTICS_URL}/regions`);
    },

    getRegistrationRatesByYear: async (year: number): Promise<AxiosResponse<MonthCountResponse[], any>> => {
  ...
  appointment: {
    ...
    getByDoctor: async (doctorId: number): Promise<AxiosResponse<Appointment[]>> => {
...
const formatParameters = (parameters: FilterParameters) => {
  if (parameters.registrationDate) {
    parameters.registrationDateGt = ((parameters.registrationDate[0] as unknown) as string).substring(
      0,
      10
    );
    parameters.registrationDateLt = ((parameters.registrationDate[1] as unknown) as string).substring(
      0,
      10
    );
