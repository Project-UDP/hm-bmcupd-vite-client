...
export const AppointmentPage = (): JSX.Element => {
  ...
  const dateCellRender = (value: Dayjs) => {
    ...
    appointments?.forEach((item) => {
      ...
      const ifDatesCorrespounding = date.year() === value.year() && date.month() === value.month() && date.date() === value.date()
  ...
  return (
    <div className="modalling d-flex align-items-center justify-content-center">
      <div className="container">
        ...
        <div className="row">
          <div className="col-md-6">
            <DatePicker showTime onChange={onChange} className="mb-2" style={{ width: '100%' }} />
          </div>
          <div className="col-md-6">
            <button disabled={!appointmentForm.dateTime || !appointmentForm.title} onClick={handleAddAppointment} className="btn btn-primary mb-2" style={{ width: '100%' }}>
              Добавить
            </button>
          </div>
        </div>
        <div className="row">
          <div className="col-md-6">
            <button onClick={handleModalClose} className="btn btn-secondary" style={{ width: '100%' }}>
              Закрыть
            </button>
          </div>
        </div>
      ...
      <div className="container">
        <div className="text-center">
          <div className="form-group border rounded p-1">
            <label htmlFor="title" className="mb-2"><h5>ФИО:</h5></label>
            <p className="mb-4">{appointment.title}</p>
          </div>
          <div className="form-group border rounded p-1">
            <label htmlFor="phoneNumber" className="mb-2"><h5>Номер телефона:</h5></label>
            <p className="mb-4">{appointment.phoneNumber}</p>
          </div>
          <div className="form-group border rounded p-1">
            <label htmlFor="dateTime" className="mb-2"><h5>Дата и время:</h5></label>
            <p className="mb-4">{appointment.dateTime}</p>
          </div>
          <div className="form-group border rounded p-2">
            <label htmlFor="additionalInformation" className="mb-2"><h5>Дополнительная информация:</h5></label>
